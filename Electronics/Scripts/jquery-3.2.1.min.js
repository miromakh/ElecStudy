...
jQuery.autocomplete = function (input, options)
{
	...
	function selectItem(li)
	{
		...
		prev = v;
		$results.html("");
		$input.val(liData ? liData.Title ? liData.Title : v : v);
		hideResultsNow();
		if (options.onItemSelect) setTimeout(function () { options.onItemSelect(liData) }, 1);
	};
	...
	function showResults()
	{
		...
		$("*").each(function ()
		{
			...
			if (zIndex > hzIndex) hzIndex = zIndex;
		});
		...
		var width = max(parseInt(iWidth), parseInt($("ul", $results).width()));
		$results.css({
			width: width + "px",
			left: pos.x + ($.page.direction() == "ltr" ? 0 : (iWidth - width)) + "px"
		});
	};
	...
	{
		if (!data) return null;
		//var parsed = []; // commented by MM on May 12, 2017. 
	...
	function dataToDom(data)
	{
		...
		for (var i = 0; i < num; i++) {
			...
			if (options.formatItem) {
				...
				li.selectValue = row.ID; //row[0];
			}
			else {
				$(li).append($("<span />").html((row.Type == "A"?"כתבה":"נושה") + " - <strong>" + row.Title + "</strong>")).data(row); // row[0];
				li.selectValue = row.ID; // row[0];
			}
			var extra = null;
			if (row.length > 1) {
